...
std::string generate_tunnel_script(const TunnelScriptContext &context) {
  ...
  ss << "    echo \"[EXV-TIMING] scope=tunnel.macos stage=$1 delta_ms=$DELTA_MS total_ms=$TOTAL_MS\"\n";
  ss << "}\n\n";
...
void cleanup_tunnel_routes(const TunnelScriptContext &context) {
  for (const auto &route : context.custom_routes) {
    platform::run_command("route -n delete " + platform::shell_quote(route) +
                       " >/dev/null 2>&1");
  }
  for (const auto &ip : context.server_route_exceptions) {
    platform::run_command("route -n delete " + platform::shell_quote(ip) +
                       " >/dev/null 2>&1");
  }
}
