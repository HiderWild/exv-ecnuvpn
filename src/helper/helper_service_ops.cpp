...
bool send_request_for_platform(const nlohmann::json& request,
                               ...
                               int /*timeout_seconds*/) {
    nlohmann::json result = exv::platform::send_helper_request(request);
    if (!result.is_object()) {
        if (error_message)
...
bool wait_until_available_for_platform(int attempts, unsigned int delay_us) {
    for (int i = 0; i < attempts; ++i) {
        ...
        if (i + 1 < attempts && delay_us > 0) {
            exv::platform::sleep_ms(static_cast<int>(delay_us / 1000));
        }
    }
    return false;
}

exv::platform::HelperServiceManagerContext service_manager_context() {
    return exv::platform::HelperServiceManagerContext{
...
public:
    InstallServiceResponse install_service(
        ...
        (void)request;
        const int exit_code = exv::platform::install_helper_service(
    ...
    UninstallServiceResponse uninstall_service(
        ...
        (void)request;
        const int exit_code = exv::platform::uninstall_helper_service(
