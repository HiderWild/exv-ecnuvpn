...
try {
  ...
  Invoke-Step cmake --build --preset windows-release --target exv exv-helper platform_status_models_test backend_resolver_test vpn_runtime_test tunnel_script_contract_test app_api_runtime_policy_test crypto_roundtrip_test

  Write-Host '[merge-prep] Run focused native regression tests...'
  Invoke-Step ctest --preset windows-release -R 'platform_status_models_test|backend_resolver_test|vpn_runtime_test|tunnel_script_contract_test|app_api_runtime_policy_test|crypto_roundtrip_test' --output-on-failure
}
finally {
  Pop-Location
}
