...
|----|--------------|-----------------------------------|------|
| 登录路径 | `POST /` aggregate-auth init/auth-reply | `POST /`，body 为 `<config-auth client="vpn" type="init">` | `production_transport.cpp` |
| 认证表示 | XML `<config-auth>` / `<auth>` / `<session-token>` | XML `<config-auth>` / `<auth>` / `<session-token>` | `aggregate_auth.cpp` |
| 会话 cookie | `webvpn=` | `webvpn=`（来自 `<session-token>`） | `auth.cpp:457` |
| CSTP 路径 | `CONNECT /CSCOSSLC/tunnel` | `CONNECT /CSCOSSLC/tunnel` | `production_transport.cpp` |
| 必备请求头 | init/auth 已发送 `X-Transcend-Version: 1`、`X-Aggregate-Auth: 1`、`Accept-Encoding: identity` | 持续与真实网关抓包核对 | `production_transport.cpp` |
...
| DTLS | 生产基线为 CSTP-only，不发送 `X-DTLS-*` 请求头 | 旧式 Master-Secret 或现代 X-DTLS12-CipherSuite/PSK 属未来后端 | `native_dtls_transport_test.cpp` |
| `extra_args` | 原生引擎拒绝未支持参数，包括 `--csd-wrapper` | 不执行任意 CLI 参数或 wrapper | `native_engine_config_mapper.cpp` |
| 客户端标识伪装 | 所有请求使用配置的 User-Agent 或默认 native UA | 所有请求统一使用配置的 AnyConnect User-Agent | `production_transport.cpp` |
...
- DTLS 加速（旧式 + 现代）是未来后端范围；当前生产基线明确为 CSTP-only。
...
- 必须发送 `CONNECT /CSCOSSLC/tunnel HTTP/1.1`，并携带：
  ...
  - 当前生产基线不发送 `X-DTLS-*` 请求头；未来 DTLS 后端见 §5。
...
当前 native-only cutover 明确支持 CSTP-only。即使服务器在 CSTP 响应中下发
`X-DTLS-*` 元数据，客户端也不得声称生产 DTLS 已启用，不得在 CONNECT 请求中发送
`X-DTLS-*` 请求头。DTLS 不可用是非致命状态，CSTP 继续运行。
...
| `disable_dtls` | 生产基线强制 CSTP-only | 未来 DTLS 后端加入后再控制 DTLS 协商（见 §5） |
