...
- [x] Confirm logs/status/config do not wait for VPN connect completion.
- [x] Confirm first-failure branch errors reach the UI before slower branch cleanup finishes.
- [x] Confirm user cancellation during connect does not trigger an error modal or failure log.
- [x] Confirm rapid connect/cancel clicking coalesces to the latest user intent.
- [x] Confirm connect timing shows separate backend/helper, platform, protocol-handshake, and serial-tail branches.
- [x] Confirm final UI mode equals the last accepted click across the interactive rapid-toggle envelope.
- [ ] Confirm real connect failure is visible to the user.
...
2026-06-19 packaged UI mode partial evidence: UIA-based toggle stress was rejected as unreliable because the WebView accessibility tree can expose only panes in compact mode and UIA traversal can block. A Win32 `SendInput` calibration pass hit the visible advanced toggle center at approximately `(1142,106)` and the minimal toggle center at approximately `(316,73)`. The calibrated run recorded accepted toggle sequences of 1x/2x at 900 ms and 8x/9x at 450 ms with final window mode matching the expected last accepted toggle, while the process stayed `Responding=True`. Faster 220 ms/120 ms/60 ms physical bursts are not interpreted by raw click-count parity because the 340 ms mode-transition overlay intentionally captures input during resize. A 30-sample idle stability probe after the stress recorded `378x148` for all samples over 2.9 seconds with `Responding=True`, proving no delayed post-click bounce in that run. Full interactive rapid-toggle parity remains unchecked.

2026-06-19 rapid-toggle envelope partial evidence: a packaged Win32 `SendInput` run recorded `mode-toggle-rapid-envelope-summary.csv`, `mode-toggle-rapid-envelope-clicks.csv`, and `mode-toggle-rapid-envelope-idle.csv` under `build/webview-acceptance/windows/`. The 450 ms accepted baselines produced 8 observed transitions ending advanced and 9 observed transitions ending minimal, with zero idle size changes. Faster shielded bursts at 150/120/80/40 ms produced fewer observed transitions because the resize shield intentionally captured physical clicks during the 340 ms settling window, but every burst still recorded zero idle size changes and `Responding=True`. This strengthens the no-bounce evidence but does not close full interactive rapid-toggle parity because the accepted-click boundary during very fast physical bursts still needs user-facing product decision/verification.

2026-06-19 connect failure visibility partial evidence: `pnpm --dir webui exec node scripts/run-host-test.cjs host/__tests__/ui-mode-and-connect-failure.test.ts` passed 5/5, including the contract that backend connection failures remain visible while `user_cancelled` errors are suppressed. A direct `./build-windows/cpp/exv.exe desktop-rpc vpn.connect "{}"` quick-validation probe returned `ok=false`, `code=auth_failed`, `error="VPN password is not configured."`, and wrote `connect-timing` `finish.error stage=quick_validation` to the runtime log. This proves the frontend-visible failure contract and a concrete desktop quick-validation failure envelope, but it is not a real gateway authentication attempt; real connect failure visibility remains unchecked.

2026-06-19 async connect-failure UI correction evidence: a new host contract first failed because `VpnStatus` did not expose the desktop status `error`, `error_code`, and `error_recoverable` fields and `applyStatus()` did not map asynchronous failed controller snapshots to the user-facing error dialog. The renderer now consumes `process_running=false` plus `error_code` during `connectInFlight`, stops connect polling/progress, and calls `setError(normalizeError(...))` while still suppressing `user_cancelled`. Verification passed: `pnpm --dir webui test:host` and `pnpm --dir webui exec vue-tsc -b`.

2026-06-19 lane isolation refresh evidence: `core_process_lifecycle_test` E2.3 now sends `logs.list`, `status.get`, and `config.getSettings` while a deterministic `vpn.connect` test hook keeps the background connection job blocked. The test asserts each read-only request responds within 500 ms before the connect hook is released. Fresh verification: `cmake --build --preset windows-release --target core_process_lifecycle_test` and `ctest --test-dir build-windows/cpp -R "core_process_lifecycle_test" --output-on-failure` passed.

2026-06-19 connect workflow contract refresh evidence: `cmake --build --preset windows-release --target connect_pipeline_test connect_intent_test vpn_actions_test app_api_status_contract_test` and `ctest --test-dir build-windows/cpp -R "connect_pipeline_test|connect_intent_test|vpn_actions_test|app_api_status_contract_test" --output-on-failure` passed. These tests cover first-failure returning before slow branches release, cancellation requests for losing branches, late non-cancel failure logging with discarded reason, user-cancelled disconnect without last error, rapid connect/cancel coalescing to the latest intent, one reconnect after cleanup for newer connect intent, no retry without newer epoch after failure, `vpn.disconnect` during connect returning `user_cancelled=true`, and source contract guards for backend/helper, platform, protocol-handshake, first-failure, and serial-tail timing markers.

2026-06-19 manual capture support: `scripts/manual-phase7-vpn-verification.ps1` now provides a local Phase 7 evidence collector for the remaining interactive checks. It can launch the packaged Windows UI, or run with `-NoLaunch` while an existing UI is already open, samples `exv-ui`/`exv`/`exv-helper` process responsiveness without process command lines, records a redacted connect-stage log summary by default, and writes an observation template under `build/manual-verification/`. Raw current-session log delta capture is opt-in via `-IncludeRawLogDelta` and is redacted before writing. This is a capture aid only; the real interactive VPN/UI items remain unchecked until a human fills the observation file from an actual stuck/failure session.

2026-06-19 manual capture RPC-probe enhancement: `scripts/manual-phase7-vpn-verification.ps1 -ProbeRpc` now records read-only `status.get` and `logs.list` desktop-RPC latency to `rpc-probes.csv` during the manual observation window, including timeout, exit code, status `error_code`, and log item count without writing log message bodies. It resolves `exv.exe` from the packaged directory or falls back to `build-windows/cpp/exv.exe`, and `-ExvPath` can override it. Smoke evidence: `powershell -NoProfile -ExecutionPolicy Bypass -File scripts\manual-phase7-vpn-verification.ps1 -NoLaunch -ProbeRpc -MonitorSeconds 1 -SampleIntervalMs 250 -OutputRoot build\manual-verification-smoke` produced `status.get=91 ms`, `logs.list=38 ms`, `timed_out=False` for both probes, and `logs.list item_count=100`. `gpt-5.3-codex-spark` also rechecked the two remaining Phase 7 items and agreed they must stay unchecked because fake AnyConnect tests cover protocol/error-code behavior but not real packaged UI interaction with a live gateway.

2026-06-19 oneshot helper leak correction evidence: manual capture found multiple long-lived `exv-helper.exe` processes after failed/cancelled one-shot connect attempts. `desktop.connect.backend_helper_ready` now records the started helper pid, self-cleans an already-started one-shot helper if another branch has cancelled the pipeline, cleans an unused one-shot backend on first-failure and serial-tail pre-controller cancellation, and resets the controller holder if cancellation arrives after `ensure_tunnel_controller(helper_endpoint)` but before controller configuration. Verification: `cmake --build --preset windows-release --target exv app_api_status_contract_test connect_pipeline_test vpn_actions_test connection_attempt_test core_process_lifecycle_test core_architecture_contract_test no_secret_in_logs_test` and `ctest --test-dir build-windows/cpp -R "core_process_lifecycle_test|core_architecture_contract_test|no_secret_in_logs_test|app_api_status_contract_test|connect_pipeline_test|vpn_actions_test|connection_attempt_test" --output-on-failure` passed. Correction report: `docs/superpowers/reports/2026-06-19-oneshot-helper-leak-correction.md`.

2026-06-19 disconnected status responsiveness correction evidence: a packaged Windows UI/core run reproduced perceived UI sluggishness without Windows marking the window non-responsive. Direct core pipe probes showed `status.get` taking 2602 ms while `logs.list` took 21 ms and `runtime.status` took 28 ms. Root cause was `disconnected_status()` filling stable upstream-virtual fields and then still calling `virtual_network::add_status_fields(j, "")`, which launches the Windows `Get-NetAdapter/Get-NetRoute` probe even when disconnected. The Dashboard page also duplicated the app-shell status refresh already performed by `App.vue`. Spark follow-up review found a real remaining controller path: `status.get` can still enter `frontend_status_from_controller_snapshot()` when a controller holder remains after idle/failed states. The status presenter now fills stable default virtual-network fields unless the snapshot/helper payload is network-ready and has a concrete interface name, and `desktop_status_presenter_test` uses a real probe hook to assert `disconnected_status()` plus idle/failed controller snapshots do not probe while connected snapshots still probe once. Final packaged run measured `status.get` at 101 ms, `logs.list` at 19 ms, and `runtime.status` at 27 ms with `exv-ui` and `exv` both `Responding=True` and no `exv-helper` process. Verification: `cmake --build --preset windows-release --target desktop_status_presenter_test app_api_status_contract_test`, `ctest --test-dir build-windows/cpp -R "desktop_status_presenter_test|app_api_status_contract_test" --output-on-failure`, `ctest --test-dir build-windows/cpp -R "desktop_status_presenter_test|app_api_status_contract_test|core_process_lifecycle_test|ui_shell_core_rpc_client_test|win32_webview2_runtime_test" --output-on-failure`, `pnpm --dir webui exec vue-tsc -b`, `pnpm --dir webui test:host`, `git diff --check`, and `powershell -NoProfile -ExecutionPolicy Bypass -File scripts\build-windows.ps1 desktop` passed.

2026-06-19 accepted-toggle envelope final evidence: a fresh packaged Win32 `SendInput` run on the current package generated `build/webview-acceptance/windows/mode-toggle-accepted-envelope-20260619-130056-summary.csv`, `mode-toggle-accepted-envelope-20260619-130056-clicks.csv`, and `mode-toggle-accepted-envelope-20260619-130056-idle.csv`. The run covered 8 and 9 accepted baseline clicks at 450 ms plus shielded 8/12/16/24 physical-click bursts at 150/120/80/40 ms. For each combination, final mode equalled `expected_last_accepted`, `idle_changes=0`, and `Responding=True`; an independent assertion over the summary CSV printed `accepted-envelope assertions passed`. This closes the final-mode item under the defined product behavior that clicks captured by the 340 ms resize shield are not accepted transitions.
