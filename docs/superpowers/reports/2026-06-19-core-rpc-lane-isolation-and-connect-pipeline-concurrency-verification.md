...
- Oneshot helper leak correction: `cmake --build --preset windows-release --target exv app_api_status_contract_test connect_pipeline_test vpn_actions_test connection_attempt_test core_process_lifecycle_test core_architecture_contract_test no_secret_in_logs_test` passed, and `ctest --test-dir build-windows/cpp -R "core_process_lifecycle_test|core_architecture_contract_test|no_secret_in_logs_test|app_api_status_contract_test|connect_pipeline_test|vpn_actions_test|connection_attempt_test" --output-on-failure` passed. See `docs/superpowers/reports/2026-06-19-oneshot-helper-leak-correction.md`.
...
- A fresh accepted-toggle envelope run on the current package generated `build/webview-acceptance/windows/mode-toggle-accepted-envelope-20260619-130056-summary.csv`, `mode-toggle-accepted-envelope-20260619-130056-clicks.csv`, and `mode-toggle-accepted-envelope-20260619-130056-idle.csv`. It covered 8/9 accepted baseline clicks at 450 ms and shielded 8/12/16/24 physical-click bursts at 150/120/80/40 ms. Every combination ended in `expected_last_accepted`, recorded `idle_changes=0`, and stayed `Responding=True`; an independent assertion over the summary CSV printed `accepted-envelope assertions passed`. This verifies final mode against the product definition that resize-shielded physical clicks are not accepted transitions.
...
- Follow-up correction: asynchronous controller failures surfaced through `status.get` are now consumed by the renderer. `VpnStatus` includes `error`, `error_code`, and `error_recoverable`; during `connectInFlight`, a `process_running=false` status with a non-`user_cancelled` `error_code` stops connect polling/progress and opens the normalized error dialog. Verification: the new `ui-mode-and-connect-failure.test.ts` contract failed before the fix, then `pnpm --dir webui test:host` and `pnpm --dir webui exec vue-tsc -b` passed.
...
- Unused one-shot helper cleanup is now covered by `app_api_status_contract_test`: backend helper readiness records the helper pid, self-cleans when cancellation arrives after one-shot startup, first-failure and serial-tail pre-controller cancellation clean the unused one-shot endpoint, and post-controller pre-config cancellation resets the controller holder.
- Disconnected and idle/failed controller `status.get` paths no longer run the Windows virtual-network platform probe. A packaged Windows run before the correction measured `status.get=2602 ms`, `logs.list=21 ms`, and `runtime.status=28 ms`; after removing the redundant disconnected/controller-idle probes and the duplicate Dashboard mount refresh, the final packaged probe measured `status.get=101 ms`, `logs.list=19 ms`, and `runtime.status=27 ms` with `exv-ui` and `exv` both `Responding=True` and no `exv-helper` process. A follow-up `desktop_status_presenter_test` uses a real virtual-network probe hook to assert `disconnected_status()` plus idle/failed controller snapshots do not probe while connected snapshots still probe once.
...
- The same tool now supports `-ProbeRpc`, which writes `rpc-probes.csv` with read-only `status.get` and `logs.list` desktop-RPC latency, timeout state, exit code, status `error_code`, and log item count. The probe intentionally does not store log message bodies. A local smoke run with `-NoLaunch -ProbeRpc -MonitorSeconds 1` produced `status.get=91 ms`, `logs.list=38 ms`, and no probe timeouts after fixing the probe stdout/stderr reader to consume output asynchronously.
- A `gpt-5.3-codex-spark` read-only recheck agreed that the remaining two Phase 7 items cannot be closed from fake AnyConnect or contract tests: those tests cover protocol/error-code and renderer contracts, not the real packaged UI + live gateway interaction chain.
...
- Confirm rapid 8+ minimal/advanced clicks with human hands only if product wants physical-click parity instead of the current accepted-transition semantics. Automated packaged evidence verifies final mode against the current accepted-transition behavior.
