...
传统 AnyConnect 客户端常见的问题是默认接管全局网络：连接校园 VPN 后，很多普通互联网流量也会绕到 VPN 服务器，再叠加 DNS 污染或出口策略差异，容易表现为整台电脑网络变慢、部分网站打不开，代理服务或海外网站也可能受影响。另一方面，AnyConnect 的登录体验也比较笨重，不能方便地记住密码，每次连接都要重新输入。

EXV 希望把这些日常痛点处理掉：校园资源走 VPN，非校园流量继续走本地网络；常用配置和凭据交给客户端管理，减少每次连接前的重复操作。
