...
describe('native WebView package policy', () => {
  ...
  it('wraps advanced and minimal content in the shared transparent app frame', () => {
    ...
    assert.match(frameVue, /\.app-window-titlebar \{[\s\S]*position: absolute;/)
    assert.match(frameVue, /\.app-window-titlebar \{[\s\S]*background: transparent;/)
    assert.match(frameVue, /\.app-window-content-shell \{[\s\S]*height: 100%;/)
