...
describe('modal onboarding and credential contracts', () => {
  ...
  it('mounts all in-window modals through the shared clipped modal shell', () => {
    ...
    assert.match(globalStack, /<ErrorDialog\s+:compact="compactModals"\s*\/>[\s\S]*<\/template>/)
  ...
  it('renders minimal-window modals as compact text and small actions without scrolling forms', () => {
    const modalShell = readSource('src', 'components', 'ModalShell.vue')
    const globalStack = readSource('src', 'windows', 'GlobalWindowStack.vue')
    ...
    assert.match(modalShell, /compact\?:\s*boolean/)
    assert.match(modalShell, /modal-shell__panel--compact/)
    assert.match(modalShell, /\.modal-shell__panel--compact\s+\.modal-shell__body\s*\{[\s\S]*overflow:\s*hidden/)
    assert.doesNotMatch(modalShell, /max-height:\s*calc\(100vh - 16px\)[\s\S]*overflow:\s*auto/)

    assert.match(globalStack, /compactModals/)
    assert.match(globalStack, /<QuickStartDialog \/>/)
    assert.match(globalStack, /v-if="compactModals"/)
    assert.match(globalStack, /modal-compact-message/)
    ...
    assert.match(passwordPrompt, /modal-compact-form/)
    assert.match(credentialPrompt, /credentialPromptNeedsFullWindow/)
  })
  ...
  it('loads brand, default VPN values, and about metadata from the distribution config', () => {
    ...
    assert.equal(distributionJson.repository.label, 'HiderWild/exv-ecnuvpn')
    assert.equal(distributionJson.repository.url, 'https://github.com/HiderWild/exv-ecnuvpn/')

    assert.match(generated, /Generated from distribution\/ecnu\.json/)
    assert.match(generated, /brandSubtitle:\s*'for ECNU'/)
    assert.match(generated, /label:\s*'HiderWild\/exv-ecnuvpn'/)
    assert.match(generated, /url:\s*'https:\/\/github\.com\/HiderWild\/exv-ecnuvpn\/'/)
  ...
  it('keeps a guarded Windows local user config reset helper for clean-profile tests', () => {
    const resetScript = readRepoSource('scripts', 'clear-local-user-config.ps1')
    ...
    assert.match(resetScript, /exv\.log/)
    assert.match(resetScript, /exv-core-ipc-v1\.registry\.json/)
    assert.match(resetScript, /exv-core-ipc-v1\.lock/)
    assert.match(resetScript, /exv-core-ipc-v1\.sock/)
    assert.match(resetScript, /connect-attempt\.json/)
    ...
    assert.doesNotMatch(resetScript, /Stop-Process -Name exv-helper/)
    assert.doesNotMatch(resetScript, /Stop-Service/)
    assert.doesNotMatch(resetScript, /sc\.exe delete/)
    assert.match(resetScript, /Programs\\EXV/)
    ...
    assert.match(resetScript, /\[switch\]\$IncludeCredentialManager/)
  })
  ...
  it('uses one export config action with a password choice dialog and corrected icons', () => {
    ...
    assert.match(system, /importBusy/)
    assert.match(system, /exportBusy/)
    ...
    assert.match(system, /async function handleImportFileChange[\s\S]*if \(importBusy\.value\) return[\s\S]*importBusy\.value = true[\s\S]*finally \{[\s\S]*importBusy\.value = false[\s\S]*\}/)
    assert.match(system, /async function openExportDialog[\s\S]*if \(exportBusy\.value\) return[\s\S]*exportBusy\.value = true[\s\S]*finally \{[\s\S]*exportBusy\.value = false[\s\S]*\}/)
    assert.match(system, /async function submitExportConfig[\s\S]*if \(exportBusy\.value\) return[\s\S]*exportBusy\.value = true[\s\S]*catch \(error\) \{[\s\S]*closeExportDialog\(\)[\s\S]*ui\.requestError[\s\S]*finally \{[\s\S]*exportBusy\.value = false[\s\S]*\}/)
    assert.match(system, /:disabled="importBusy"/)
    assert.match(system, /:disabled="exportBusy"/)
    assert.match(system, /:disabled="exportPasswordRequired \|\| exportBusy"/)
